...
Explorer Pages is a **separate** workflow (`.github/workflows/explorer-pages.yml`) so that red job does not bury deploy status. It builds [OpenTideHQ/explorer](https://github.com/OpenTideHQ/explorer) with Node from this corpus (`OPENTIDE_REPO_ROOT`) and generates ATT&CK vocab from [OpenTideHQ/specifications](https://github.com/OpenTideHQ/specifications) (`SPECIFICATIONS_REPO_ROOT`).
